...
void  prepareLabels(float * tLables)
{
	string line;
	int i=0;
	
  ifstream myfile ("labels.txt");
  if (myfile.is_open())
  {
    while (! myfile.eof() )
    {
      getline (myfile,line);
       if(line.compare("1.000000")==0)
		 ...
		 else  if(line.compare("-1.000000")==0)
		  {
			  *(tLables+i)=-1.0;
		  }
...
int main(int argc, char** argv)
{
	...
	float **test=new float*[973];
	//trainingData=new float*[1007];
	trainingData=prepareTrainData(301,"per00", size,trainingData,8,4,i,9);
    trainingData=prepareTrainData(401,"bikeTrainingSet.txt", size,trainingData,8,4,i,"/home/saiph/281b/final-ece-cs281b/",9);
	trainingData=prepareTrainData(484,"bikeList.lst", size,trainingData,8,4,i,"/home/saiph/281b/final-ece-cs281b/test/bike/",9);   
   	test=prepareTrainData(973,"images.txt", size,test,8,4,k,"/home/saiph/281b/final-ece-cs281b/test/test2/",9);
   ...
   // trainingData=prepareTrainData(924,"per00", size,trainingData,8,4,i);
    ...
    float* linarray = imgutils::mat2linarray((const float **)trainingData,size, 484);
    float* linarrayTest = imgutils::mat2linarray((const float **)test,size, 973);
    
    for(int z=0;z<484;z++)
    {
		delete [] trainingData[z];
	}
	delete [] trainingData;
	...
	delete [] test;
	cv::Mat testMatrix(973, size, CV_32FC1,linarrayTest);
    float * testLables =new float[973];
    prepareLabels(testLables);
    
	/*
    cv::Mat tset(484, size, CV_32FC1,linarray);
