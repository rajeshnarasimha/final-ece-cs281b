...
void write(float **trainingData,int size, int numTrain,std::string nameFile);
 ...
 float ** trainEx(std::string fileName,int trainEx,float **trainingData,std::string nameFileWrite,int & size,int windowSize,int blockSize,int numBines);
  
 void prepareDerImages(std::string pictureName,cv::Mat & dx, cv::Mat & dy);
 float ** trainEx(cv::Mat originalImage,int trainEx,float **trainingData,std::string nameFileWrite,int & sizeInput,int windowSize,int blockSize,int numBines);
 void prepareDerImages(cv::Mat img,cv::Mat & dx, cv::Mat & dy); 
 
 std::vector <cv::Mat> normalizeBlock(std::vector <cv::MatND>   block, double e);
 float predictionHOG(cv::Mat,int numBlocks,int numCeldas,int numBines);
 float ** integrate( std::vector<cv::Mat>block, float  ** a, int & indice,int trainEx,int size,int windowSize);
 float ** prepareTrainData(int numTrainExample,std::string fileName, int & size,float **trainingData,int sizeCeldas,int sizeBlock,int & i,std::string ruta,int numBines);
 
 float ** prepareTrainData(int numTrainExample,std::string fileName, int & size,float **trainingData,int sizeCeldas,int sizeBlock,int & i,int numBines);

void computeBlocks(int & sizeInput,double e,std::vector< std::vector<cv::MatND> > rowsOfCells,int windowSize,const int blockSize,std::vector< std::vector <cv::Mat> > & normalizedBlocks);
